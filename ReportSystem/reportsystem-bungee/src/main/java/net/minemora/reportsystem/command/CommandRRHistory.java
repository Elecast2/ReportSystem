...
public class CommandRRHistory extends Command implements TabExecutor {

	public CommandRRHistory() {
		super("rrhistory", "staff");
	}
	...
	private void showHistory(String playerName, UUID uid, ProxiedPlayer player) {
		LinkedHashMap<Report, Long> reports = Util.sort(Database.getDatabase().getReportedHistory(uid, playerName));
		
		if(reports.size() == 0) {
			player.sendMessage(TextComponent.fromLegacyText(Chat.format("&4" + playerName + " &cno ha sido reportado.")));
			return;
		}
